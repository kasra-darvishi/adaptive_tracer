#!/bin/bash
#SBATCH --job-name=lstm_event_cats3_512
...
LOG_DIR=$PROJECT/logs/lstm_event_cats3_seq512_${SLURM_JOB_ID}
...
echo "Mode         : LSTM | Event-only | paper bins=3 | seq=512"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "lstm_event_cats3_seq512_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
