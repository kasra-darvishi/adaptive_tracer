#!/bin/bash
#SBATCH --job-name=trf_duration_cats3_512
...
DATA=$WORKROOT/micro-service-trace-data/preprocessed_lmat_kernel_cats3_seq512
LOG_DIR=$PROJECT/logs/transformer_duration_cats3_seq512_${SLURM_JOB_ID}
...
echo "Mode         : Transformer | Duration-only | paper bins=3 | seq=512"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "transformer_duration_cats3_seq512_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
