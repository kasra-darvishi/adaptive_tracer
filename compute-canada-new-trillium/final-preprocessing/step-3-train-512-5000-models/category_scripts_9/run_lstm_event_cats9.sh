#!/bin/bash
#SBATCH --job-name=lstm_event_cats9_512
...
LOG_DIR=$PROJECT/logs/lstm_event_cats9_seq512_${SLURM_JOB_ID}
...
echo "Mode         : LSTM | Event-only | paper bins=9 | seq=512"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "lstm_event_cats9_seq512_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
