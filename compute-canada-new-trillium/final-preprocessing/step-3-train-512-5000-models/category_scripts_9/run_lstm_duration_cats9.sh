#!/bin/bash
#SBATCH --job-name=lstm_duration_cats9_512
...
LOG_DIR=$PROJECT/logs/lstm_duration_cats9_seq512_${SLURM_JOB_ID}
...
echo "Mode         : LSTM | Duration-only | paper bins=9 | seq=512"
...
python -u microservice/train_sockshop.py \
  ...
  --wandb_run_name "lstm_duration_cats9_seq512_${SLURM_JOB_ID}" \
  --log_dir "$LOG_DIR" \
  --gpu 0
