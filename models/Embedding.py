...
class Embedding(nn.Module):
    def __init__(
        ...
        if self.dim_proc > 0:
            self.emb_proc = nn.Embedding(n_process, self.dim_proc, padding_idx=0)
        # if self.dim_f_mean > 0:
        #     self.emb_f_mean = nn.Embedding(n_category, self.dim_f_mean, padding_idx=0)
    ...
    def init_weights(self):
        ...
        # if self.dim_f_mean > 0:
        #     nn.init.normal_(self.emb_f_mean.weight, 0, self.dim_f_mean**-0.5)
    ...
    def forward(self, call, entry, ret, time, proc, pid, tid):
        ...
        # if self.dim_f_mean > 0:
        #     emb = torch.cat((emb, self.emb_f_mean(f_mean)), -1)
